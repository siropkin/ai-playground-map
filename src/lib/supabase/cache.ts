 ...
 */
export async function getDescriptionFromCache(address: string): Promise<string | null> {
  try {
...
export async function saveDescriptionToCache(address: string, description: string): Promise<void> {
  try {
    const supabase = await createClient();
 ...
 */
export async function getOSMDetailsFromCache(osmType: string, osmId: string): Promise<OSMPlaceDetails | null> {
  try {
    const supabase = await createClient();
...
export async function saveOSMDetailsToCache(detail: OSMPlaceDetails): Promise<void> {
  try {
    const supabase = await createClient();
...
export async function getMultipleOSMDetailsFromCache(items: { id: string; type: string }[]): Promise<{
  ...
  try {
    ...
    for (let i = 0; i < items.length; i += batchSize) {
      ...
      const filterConditions = batch.map(item => {
      ...
      for (const item of batch) {
        const cacheEntry = data.find(entry => 
          entry.osm_type === item.type && entry.osm_id === item.id
        );
...
export async function saveMultipleOSMDetailsToCache(details: OSMPlaceDetails[]): Promise<void> {
  try {
